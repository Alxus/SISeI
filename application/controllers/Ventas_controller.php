...
class Ventas_controller extends CI_Controller {
	...
	public function Abono(){
		...
		if($this->input->post('fb')=='' && $this->Asistentes_model->getbyid($this->input->post('email'))==null){
			...
			if($result['affected_rows']==1){
				...
				if($abono['estado']=='PAGADO'){
					if(($this->Pagos_model->getPagados()[0]['total']+$this->Asistentes_model->getPagados()[0]['total'])<50)
						$asistente['pro']=true;
				}
				if($this->Asistentes_model->abono_Asistente($abono)){
